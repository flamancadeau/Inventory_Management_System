...
// Fetch the data for the product based on the ID
$res = mysqli_query($link, "SELECT * FROM import_ketchen WHERE id='$id'");
...
<div id="content">
    <div id="content-header">
        <div id="breadcrumb"><a href="" class="tip-bottom"><i class="icon-home"></i> Edit kitchen Product</a></div>
    </div>
...
if (isset($_POST['submit1'])) {
    ...
    mysqli_query($link, "UPDATE `import_ketchen` SET `product_name`='$new_product_name', `unit`='$unit', `Price`='$Price', `Date`='$Date' WHERE id='$id'") or die(mysqli_error($link));

    // Fetch the current stock quantities for comparison
    $import_res = mysqli_query($link, "SELECT Quantity FROM `import_ketchen` WHERE id='$id'");
    ...
    $served_res = mysqli_query($link, "SELECT SUM(Quantity) as total_served FROM `served_ketchen` WHERE product_name='$new_product_name'");
    ...
    if ($total_served_quantity > $current_import_quantity) {
        ...
        mysqli_query($link, "UPDATE `import_ketchen` SET `Quantity`='$new_Quantity' WHERE id='$id'") or die(mysqli_error($link));
    } else {
        ?>
...
<script type="text/javascript">
    ...
    <script type="text/javascript">
        ...
        setTimeout(function () {
            window.location = "add_kitchen_products.php";
        }, 1000);
    </script>
    <?php
}
