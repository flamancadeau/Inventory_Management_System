...
$res=mysqli_query($link,"select * from import_bar where id='$id' ");
...
if (isset($_POST['submit1'])) {
    ...
    mysqli_query($link, "UPDATE `import_bar` SET `product_name`='$new_product_name', `unit`='$unit', `Price`='$Price', `Date`='$Date' WHERE id='$id'") or die(mysqli_error($link));

    // Fetch the current stock quantities for comparison
    $import_res = mysqli_query($link, "SELECT Quantity FROM `import_bar` WHERE id='$id'");
    ...
    if ($total_served_quantity > $current_import_quantity) {
        ...
        mysqli_query($link, "UPDATE `import_bar` SET `Quantity`='$new_Quantity' WHERE id='$id'") or die(mysqli_error($link));
    } else {
        ?>
...
<script type="text/javascript">
    ...
    <script type="text/javascript">
        ...
        setTimeout(function () {
            window.location = "add_bar_products.php";
        }, 1000);
    </script>
    <?php
}
