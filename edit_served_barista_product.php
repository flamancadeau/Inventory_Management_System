...
$res=mysqli_query($link,"select * from served_barista where id='$id' ");
...
<div id="content">
    ...
    <div id="content-header">
        <div id="breadcrumb"><a href="" class="tip-bottom"><i class="icon-home"></i>
            Edit served Barista Product</a></div>
    </div>
    <!--End-breadcrumbs-->
...
if (isset($_POST['submit1'])) {
    ...
    mysqli_query($link, "UPDATE `served_barista` SET `product_name`='$new_product_name', `unit`='$unit', `Price`='$Price', `Date`='$Date' WHERE id='$id'") or die(mysqli_error($link));

    // Fetch the current stock quantities for comparison
    $import_res = mysqli_query($link, "SELECT Quantity FROM `served_barista` WHERE id='$id'");
    ...
    if ($total_served_quantity > $current_import_quantity) {
        ...
        mysqli_query($link, "UPDATE `served_barista` SET `Quantity`='$new_Quantity' WHERE id='$id'") or die(mysqli_error($link));
    } else {
        ?>
