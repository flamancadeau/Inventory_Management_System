...
<div id="content">
    <div id="content-header">
        <div id="breadcrumb"><a href="" class="tip-bottom"><i class="icon-home"></i> Add Kitchen Product</a></div>
    </div>
...
<div class="control-group">
        ...
        <datalist id="units">
            ...
            ?>
                            ...
                            <div class="form-actions">
                                <button type="submit" name="serve_product" style=" background: #6F4E37;border-radius:5px" class="btn btn-success">Serve Product</button>
                            </div>
  ...
  <form method="post" action="export_excel_served_ketchen.php">
                <input type="hidden" name="table_name" value="served_ketchen"> <!-- Table name -->
                <input type="hidden" name="filename" value="export kitchen records"> <!-- Desired file name -->
    ...
    <a></a><i class="fas fa-file-excel"></i> </a>
            ...
            <div class="widget-content nopadding">
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        <?php
                        $res = mysqli_query($link, "SELECT * FROM served_ketchen");
                        while ($row = mysqli_fetch_array($res)) {
                            echo "<tr>
                             <td>{$row['id']}</td>
...
<td>
    <center>
        <a title='Edit' class='tip-bottom' href='edit_served_kitchen_product.php?Date=" . urlencode($row['Date']) . "' style='color: green'><i class='icon icon-edit'></i></a>
        &nbsp;&nbsp;&nbsp;&nbsp;
    </center>
</td>
...
if (isset($_POST['serve_product'])) {
    ...
    $stock_res = mysqli_query($link, "SELECT SUM(Quantity) as total_stock FROM import_ketchen WHERE id='$id'");
    ...
    $served_res = mysqli_query($link, "SELECT SUM(Quantity) as total_served FROM served_ketchen WHERE id='$id'");
    ...
    if ($remaining_stock >= $Quantity) {
        ...
        mysqli_query($link, "INSERT INTO served_ketchen (product_name, unit, Quantity, Price, Date,id) VALUES ('$product_name', '$unit', '$Quantity', '$Price', '$Date','$id')");
